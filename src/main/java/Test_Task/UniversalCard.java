...
public class UniversalCard {
    ...
    public DrivingLicense getDrivingLicense(String unicId){
        ...
        log.warning("Driving license is not found (for unicId)");
        return null;
    }

    public DrivingLicense getDrivingLicense(int index) {
        if (index < drivingLicenses.size()) {
            log.info("Driving license found and returned (for index)");
            return drivingLicenses.get(index);
        }
        else {
            log.warning("Driving license is not found and can't returned (for index)");
            return null;
        }
    }

    public void removeDrivingLicense(String unicId){
        for (int i = 0; i < drivingLicenses.size(); i++) {
            if (isEqual(drivingLicenses.get(i), unicId)) {
                log.info("Driving license found and deleted (for unicId)");
                drivingLicenses.remove(i);
            }
        }
        log.warning("Insurance policy is not found and can't deleted (for unicId)");
    }

    public void removeDrivingLicense(int index) {
        if (index < drivingLicenses.size()) {
            log.info("Credit card found and deleted (for index)");
            drivingLicenses.remove(index);
        }
        else {
            log.warning("Driving license card is not found and can't deleted (for index)");
        }
    }
    ...
    public CreditCard getCreditCard(String unicId){
        for (CreditCard item: creditCards) {
            if (isEqual(item, unicId)) {
                log.info("Credit card found and returned (for unicId)");
                return item;
            }
        }
        log.warning("Credit card is not found and can't returned (for unicId)");
        return null;
    }
    public CreditCard getCreditCard(int index) {
        if (index < creditCards.size()) {
            log.info("Credit card found and returned (for index)");
            return creditCards.get(index);
        }
        else {
            log.warning("Credit card is not found and can't returned (for index)");
            return null;
        }
    }


    public void removeCreditCard(String unicId){
        for (int i = 0; i < creditCards.size(); i++) {
            if (isEqual(creditCards.get(i), unicId)) {
                log.info("Credit card found and deleted (for unicId)");
                creditCards.remove(i);
            }
        }
        log.warning("Credit card is not found and can't deleted (for unicId)");
    }
    public void removeCreditCard(int index) {
        if (index < creditCards.size()) {
            ...
            log.warning("Credit card is not found and can't deleted (for index)");
        }
    }
    ...
    public InsurancePolicy getInsurancePolicy(String unicId){
        for (InsurancePolicy item: insurancePolicies) {
            if (isEqual(item, unicId)) {
                    log.info("Insurance policy found and returned (for unicId)");
                    return item;
                }
            }
        log.warning("Insurance policy not found and can't returned (for unicId)");
        return null;
    }

    public InsurancePolicy getInsurancePolicy(int index) {
        if (index < insurancePolicies.size()){
                log.info("Insurance policy found and returned (for index)");
                return insurancePolicies.get(index);
            }
            else {
                log.warning("Insurance policy not found and can't returned (for index)");
                return null;
            }
    }

    public void removeInsurancePolicy(String unicId){
        for (int i = 0; i < insurancePolicies.size(); i++) {
            if (isEqual(insurancePolicies.get(i), unicId)) {
                log.info("Insurance policy found and deleted (for unicId)");
                insurancePolicies.remove(i);
            }
        }
        log.warning("Insurance policy not found and can't deleted (for unicId)");
    }

    public void removeInsurancePolicy(int index) {
        if (index < insurancePolicies.size()){
            log.info("Insurance policy found and deleted (for index)");
            insurancePolicies.remove(index);
        }
        else {
            log.warning("Insurance policy not found and can't deleted (for index)");
        }
    }
